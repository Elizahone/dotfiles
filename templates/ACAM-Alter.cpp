...
void setFailPtr() {
	...
	for (int i = 0; i < 10; i++) {
		...
		fail[Trie[u][i]] = Trie[fail[u]][i]; // 如果这条路存在那么子节点的fail指针就时父节点的fail指针所指向的节点的 i号路，并且这条路一定已经是直达的，因此这个一定在该更上层
		box[r++] = Trie[u][i];
	    }
	}
	// 不一定需要
...
int f(int i, int j, int free, int has) {
    if (alter[j]) return 0;
    if (i == n) return has;
    if (memo[i][j][free][has] != -1) return memo[i][j][free][has];
    int ans = 0;
    int cur = num[i]-'0';
    if (has == 0) {
	if (free == 0) {
...
void solve() {
    ...
    for (int i = 0; i < 1301; i++) for (int j = 0; j < 3002; j++) memo[i][j][0][0] = -1, memo[i][j][0][1] = -1, memo[i][j][1][0] = -1, memo[i][j][1][1] = -1;
